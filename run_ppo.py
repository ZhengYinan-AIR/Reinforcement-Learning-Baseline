'''
https://github.com/Lizhi-sjtu/DRL-code-pytorch/blob/main/5.PPO-continuous/PPO_continuous_main.py
'''
import os
import torch
import numpy as np
import gym
import argparse
from utils.util import Normalization, RewardScaling
...
def evaluate_policy(config, env, agent, state_norm):
    ...
    for _ in range(times):
        ...
        episode_reward = 0
        while not done:
            a = agent.step(s)  # We use the deterministic policy during the evaluating
            #print(a)
...
def run(config):
    ...
    seed = config['seed']
    exp_name = config['algo'] + '_' + config['env_name']
...
if __name__ == '__main__':
    ...
    parser.add_argument("--hidden_sizes", type=int, default=256, help="The number of neurons in hidden layers of the neural network")
    parser.add_argument("--actor_lr", type=float, default=3e-4, help="Learning rate of actor")
    parser.add_argument("--lr", type=float, default=3e-4, help="Learning rate of critic")
